...
| DEC-0018 | First provider enablement: three mutually exclusive options (A: Anthropic API, two pinned models, synthetic-only, conservative ceilings — recommended; B: OpenAI API cheapest configuration with added storage/cap risks; C: preserve DEC-0009) prepared from official-source research (decision packet + threat model, all sources accessed 2026-07-19); grants no authority while proposed; awaits Product Owner ratification (Q-010); no provider is enabled and EXP-0001 remains unexecuted | proposed |
